...
export type ClasificadasSemisHits = {
  count: number;
  teams: string[];
};

export type ClasificadasSemisHitsData = Record<string, ClasificadasSemisHits>;
...
function parseActualSemisTeams(csv: string): string[] {
  const sanitized = csv.replace(/^\uFEFF/, '').trim();
  const lines = sanitized.split(/\r?\n/).filter(Boolean);
