...
export type ClasificadasFinalHits = {
  count: number;
  teams: string[];
};

export type ClasificadasFinalHitsData = Record<string, ClasificadasFinalHits>;
...
function parseActualFinalTeams(csv: string): string[] {
  const sanitized = csv.replace(/^\uFEFF/, '').trim();
  const lines = sanitized.split(/\r?\n/).filter(Boolean);
...
export async function fetchFinalClasificadasHits(): Promise<ClasificadasFinalHitsData> {
