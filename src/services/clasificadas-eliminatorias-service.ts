...
export type ClasificadasHits = {
  count: number;
  teams: string[];
};

export type ClasificadasHitsData = Record<string, ClasificadasHits>;
...
function parseActualOctavosTeams(csv: string): string[] {
  const sanitized = csv.replace(/^\uFEFF/, '').trim();
  const lines = sanitized.split(/\r?\n/).filter(Boolean);
